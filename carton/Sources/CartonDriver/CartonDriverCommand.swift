...
func derivePackageCommandArguments(
  ...
  var packageArguments: [String] = [
    "package", "--swift-sdk", SWIFT_SDK, "--triple",
    "wasm32-unknown-wasi", 
  ...
  case "test":
    ...
    if !buildArguments.isEmpty {
      let buildCommand =
        buildArguments.map(String.init) + [
...
func pluginSubcommand(subcommand: String, argv0: String, arguments: [String]) async throws {
  ...
  print(
    "##### Running \(swiftPath.swift) with arguments: \(pluginArguments.joined(separator: " ")) -- \(ToolchainSystem.isSnapshotVersion(swiftPath.version)) -- \(swiftPath.toolchain.pathExtension)"
  )
