...
struct BundleLayout {
  ...
  func copyResources(wasmDestinationPath: AbsolutePath, terminal: InteractiveWriter) throws {
    ...
    print(
      "@@@@ Resources directory names: \(resourcesDirectoryNames), build directory: \(bundleDirectory), hasJavaScriptKitResources: \(hasJavaScriptKitResources)"
    )
    ...
    try localFileSystem.writeFileContents(
      ...
      bytes: ByteString(
        encodingAsUTF8: indexJsContent(
          mainModuleName: wasmDestinationPath.basename,
          hasJavaScriptKitResources: hasJavaScriptKitResources)
      )
    )
