...
final class ServerHTTPHandler: ChannelInboundHandler, RemovableChannelHandler {
  ...
  func channelRead(context: ChannelHandlerContext, data: NIOAny) {
    ...
    do {
      ...
      case "/main.wasm":
        print(
          "@@@@ Main: \(configuration.mainWasmPath), entrypoint: \(configuration.entrypoint.fileName)"
        )
  ...
  struct StaticResponse {
    ...
    init(
      contentType: String, contentSize: Int, body: @autoclosure @escaping () throws -> ByteBuffer
    ) {
    ...
    -> StaticResponse?
  {
    ...
    for directoryName in try FileManager.default.resourcesDirectoryNames(
      relativeTo: buildDirectory.asURL)
    {
      responders.append { context, uri in

        print("@@@ Static responder for \(directoryName) at \(uri)")
        // If the uri starts
